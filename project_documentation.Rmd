---
title: "Final Project Documentation (STAT 243): Adaptive Rejection Sampling (ARS) Package"
...
GitHub usernames of the group members are: Andrea - **andrea2910**, Dingxin - **dxcai**, and Yulun - **AeroAiro**. The final version of the project resides in the Andrea's GitHub account (see above code for instructions on package installations). 
...
When f is already semi-positive and integrable, $x_1$ (the first point) needs to be located such that $h'(x) > 0$ if the defined domain is unbounded on the left. We find this point by continuously shifting towards the left until a point that satisfies this condition is found. Note that when the base assumption, any PDF of the function is semi-positive-definite, is satisfied, there must exist a point $y: y<x$ such that $h'(y) > 0$ for any x. Likewise, if the domain is unbounded on the right, we find the initial point on the right side by continuously shifting towards the right until $h'(x) < 0$ is satisfied. 
...
The condition of ARS is that the support of the input function is connected, in the case of 1-dimension, a single interval.  However, we don't stop the process and report an error if a point x in the support is found such that $f(x) = 0$,  because it could happen that a point is sampled and the f of it is not zero analytically but is zero in R (e.g. normal distribution, $\mu=5$, $\sigma=2$, $x=100$). If such a point was found, we simply reject this point and do not update the hulls and move on. 
...
# **TESTING**
The test file in our package can be run with the following command `devtools::test()` once you are in our main folder. Similarly, the following code works if run sequentially: `library(testthat); library(ars); test_package(` The tests are broken up into 3 files:
