---
title: "Final Project Dcumentation (STAT 243): Adoptive Rejection Sampling (ARS) Package"
author: "Andrea Bonilla, Dingxin Cai, and Yulun Wu"
...
- Find the piece-wise linear upper bound, $u(y)$, to the log-density function, by evaluating the tangent lines at the fixed points at $h(y)$; the equation for finding $u(y)$ is:
  $u(y) = h(y_j) + (y-y_j)h'(y_j)$
...
![Algorithm Notation](img/notation.png)

### Validity
- The probability of x being sampled is: $P(\text{x is sampled}) = \frac{exp(u(x))}{\int exp(u(x')dx')}$

- The probability of x being accepted given x is sampled is: $P(\text{x is accepted | x is sampled}) = exp( h(x)- u(x))$ 

- By the Bayes' theorem, $P(\text{x is sampled |x is accepted})$ can be expressed as: 
\begin{align*}
\frac{ P(\text{x is sampled | x is accepted}) P(\text{x is sampled})} {\int P(\text{x is sampled | x is accepted}))P(\text{x is sampled})} &= \frac{exp(h(x)-u(x))\frac{exp(u(x))}{\int exp(u(x')dx')}}{\int exp(h(x^*)-u(x^ *))\frac{exp(u(x*))}{\int exp(u(x')dx')}dx^*} \\
&= \frac{exp(h(x))}{\int exp(h(x^*)dx^*)} \\
&= f(x)
\end{align*}
 

### ARS Package Set-up
Below is an example code chunk to set up the ars package (as well as an working example) from the assigned GitHub repository:
```{r, eval=FALSE}
...
Please note, there may be issues when you write `ars::ARS$new(funx=dnorm, mean=3000, sd=1)` instead of `ars::ARS$new(funx=dnorm, D=c(2990, 3010), mean=3000, sd=1)` once you run the sample method. 

### GitHub Usernames and Repository
GitHub user names of the group members are: Andrea - **andrea2910**, Dingxin - **dxcai**, and Yulun - **AeroAiro**. The final version of the project resides in the Andrea's GitHub account (see above code for instructions on package installations). 
...
![ARS Class Structure](img/andrea.png)


Functions for each steps in the algorithm are included in the public domain of the class. The main functions in the class are the sampling and updating functions, which are explained in detail in the next section. 
...
The figure below shows what the changes would be if the sampled point is within the supports (slightly different situation when the sampled point is at the domain boundaries). A new y value ($y_{new}$) will be added to `init_y`. The corresponding $h(y_{new})$ and $h^{'}(y_{new})$ will be added to `h_vals` and `h_prim`. The point $z_i$ will be replaced by $z_{new1}$ and $z_{new2}$ and `u_vals` will be updated accordingly. `s_cdfs` will be updated to include the two new sections, which replaced the old section between $z_i$ and $z_{i+1}$. 
...
##### Log-concavity
If we check concavity at the beginning by sampling a lot of random points within the domain to avoid falling into a pattern where there are negative points but we could never find it, we have to calculate h and h_prim values, which somewhat contradicts the efficiency purpose of squeezing test. The squeezing test allows us to only update h and h_prim as needed, and not every round of sampling. If we already calculated a lot of points' h and h_prim values, we might as well just store them and create an initial upper and lower hull which are already very close to the actual function. This is both very costly and makes the actual steps of ARS not that essential anymore.

While we do not check log-concavity initially,  we check every time a new point is added.  If a large number of points are sampled, it is very unlikely that a non-log-concave function will not be detected during the process.  But if the user is very paranoid of this incidence happening, or the result fails the KS test or they find the result to be suspicious. We also provide a squeeze=FALSE option for user to disable squeezing test,in which case even if the function is not log-concave, it would still give a theoretically valid sample of the input distribution 
if it happens that no non-log-concave upper hull is created. 
...
- Thus $F(x) \sim Unif[0,1]$, we sample from $Unif[0,1]$ and get x by $F^{-1}(u)$.
...
The condition of ARS is that the support of the input function is connected, in the case of 1-dimension, a single interval.  But we don't just stop the process and report an error if a point x in the support is found such that $f(x) = 0$,  because it could happen that a point is sampled and the f of it is not zero analytically but is zero in R (e.g. normal distribution, $\mu=5$, $\sigma=2$, $x=100$). If such a point is found, we simply reject this point and do not update the hulls and move on. 
For the case that $f(x)$ is indeed zero, it is theoretically valid to simply reject it. And the input is also not log-concave in this case, so it can be classified to the problem of detecting non-log-concavity which we discussed above. For the case that $f(x)$ is just zero in R, it happens extremely rarely (like sampling a $x=100$ from $N(5,4)$) and the effect of just rejecting it can be trivial.


# **TESTING**
The test file in our package can be ran with the following command `devtools::test()` once you are in our main folder. The tests are broken up into 3 files:

- `test_helper.R`: This makes confirms our helper functions are correct, including calculating derivatives, creating upper and lower hull functions, and checking whether our function is concave at certain points. It also confirms our `namedVector` class works the way it was intended too, which helps our ARS method.
- `test_ars_output.R`: This confirms our `ARS` function breaks when the bounds are incorrect, i.e. a exponential distribution with negative bounds, our `$sample()` method creates the correct output length, and our `ARS$new()` method breaks when a function or incorrect dimensions is not inputted
- `test_ars_ks.R`: To confirm whether the our ARS data algorithm works, we compare samples generated from our function and corresponding r method codes i.e `rnorm`, `runif`, and others. To compare the validity we use the Kolmogorov-Smirnov test to compare the sameness of our continuous distributions. 

While running tests, the code also outputs warnings, which usually say: "Input function is not normalized. The pdf does not integrate to 1". This is okay as we check whether the pdf integrates to 1, or near one with some tolerance. 


# **TEAM MEMBER CONTRIBUTIONS**
Yulun Wu was in charge of creating the R6 classes, coding most of the other necessary functions and debugging. Andrea Bonilla worked on coding, debugging and most of the testing. She was also in charge of compiling the code into the final ars package. Dingxin Cai helped debug the code and was also in charge of the majority of the documentation. 

----

# **REFERENCES**
1. W. R. Gilks, P. Wild (1992), "Adaptive Rejection Sampling for Gibbs Sampling," Applied Statistics, Vol. 41, Issue 2, 337-348.
2. C. N. Kohnena (2002), "Adaptive Rejection Sampling," Statistical Science, Duke University. [link](http://www2.stat.duke.edu/~cnk/Links/slides.pdf)
3. Inferentialist Consulting (2016), "[R] Adaptive Rejection Sampling," *blog.inferentialist.com*. [link](https://blog.inferentialist.com/2016/09/26/adaptive-sampling.html)
